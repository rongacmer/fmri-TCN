...
class fmriDataset(InMemoryDataset):
    def __init__(self, root,  num_nodes: int,graphdir:str,hypergraphpath:str,
        ...
        self.hypergraphpath = hypergraphpath
    ...
    def roizscore(self,x:torch.tensor):
        mean = np.mean(x)
        std = np.std(x)
        x = (x-mean)/std
        return x
    ...
    def __load_data(self,subject_directory,y,subject_name):
        ...
        for i in range(self.time_lenght): #遍历所有时间点
            ...
            # roi_stat = self.roizscore(roi_stat)
        ...
        hypergraph = sio.loadmat(os.path.join(self.hypergraphpath,'{}.mat'.format(subject_name)))
        hypergraph = hypergraph['edge']
        hypergraph = generate_G_from_H(hypergraph)
        G = nx.from_numpy_array(hypergraph, create_using=nx.DiGraph)  # 无向图G
        ...
        #############################################################
        print('subject:{}'.format(subject_name))  # 输出信息，完成到哪个样本的哪个时间点

        x = np.array(x)
        x = x[:,:,0] #只用平均值
        x = np.transpose(x,[1,0])
        # stata_x =
        x = torch.tensor(x,dtype=torch.float) #size：[130,90,2]
    ...
    def get(self,idx):
        ...
        threshold = 1e-5
        max_edge = self.num_nodes * self.num_nodes #边的最大数量，使用dataloader时，要保持每个tensor的长度一致
        ...
        if self.state == 'processing':
            if subject_name not in self.cache:
                ...
                # print(subject_name)
                # data['x'] = self.roizscore(data['x'].numpy())
                ...
                edge_cnt = data['edge_index'].size()[1]
                self.cache[subject_name] = [edge_index.astype(np.long),edge_attr.astype(np.float32),data['x'],edge_cnt] #载入缓存
                  # 真正边的数量
            # r = 0
                # print(self.status, r)
            # print(self.status,r)
