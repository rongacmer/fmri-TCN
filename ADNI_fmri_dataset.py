...
class fmriDataset(InMemoryDataset):
    def __init__(self, root,  num_nodes: int,niidatadir:str,
                 time_length: int = 137,clf = ['AD','NC'],Template = None,subjectdir = None,pvaluedir = None,
                 transform=None, pre_transform=None,time_slice = 137,status = 'train',test_start = 0, threshold = 100):
        '''
        fmri数据集
        :param root:数据集的存储路径
        ...
        self.niidatadir = niidatadir
        self.pvaluedir = pvaluedir
        self.clf = clf
        self.Template = Template
        self.clf_dir = {i:self.niidatadir for i in clf} #补全为绝对路径
    ...
    def __load_data(self,niifilename:str,pvaluesubject_directory,y,subject_name):
        '''

        :param niifilename: fmri文件名
        :param pvaluesubject_directory: p-value文件夹
        :param y: 标签
        :param subject_name: 文件名
        :return: deal_data: 处理后的fmri数据
        '''
        ######载入数据##########################
        data = nib.load(niifilename)
        data = data.get_fdata()
        ...
        for i in range(self.time_lenght): #遍历所有时间点
            mask = os.path.join(self.mask,'{}.nii'.format(subject_name))
            mask = nib.load(mask)
            mask = mask.get_fdata()
            # roi_stat = self.cal_roi(data[i],self.mask) #节点 size：[90,2]\
            roi_stat = self.cal_roi(data[i], mask) #结点 size:[200,2]
            if np.isnan(roi_stat).any():
                print(niifilename,roi_stat.max(),roi_stat.min())
                return None
            ####################p-value矩阵###########################
            ...
            #############################################################
            # edge_index.append(np.array(G.edges()).transpose())
            # edge_attr.append(list(nx.get_edge_attributes(G, 'weight').values()))
            ...
            # deal_data = Data(x=x,edge_index=edge_index,edge_attr=edge_attr,y=y)
            # deal_data.ts = torch.tensor([i])
            # deal_data.id = torch.tensor([subject_id])
            # subject_data.append(deal_data)
    ...
    def process(self):
        ...
        for label,cat in enumerate(self.clf):
            ...
            for s in subject_txt:
                ...
                subject_data = self.__load_data(subject_filename,p_network_dir,label,s)
...
if __name__ == '__main__':
    ...
    #     for j in dataset:
    #         print(j)
    # train = dataset[[1,2,3,4,5,6]]
    # test = dataset[:5]
    # train.transform = RandomSelectTime('train')
    # test.transform = RandomSelectTime('test')
    # kwargs_dataloader = {'num_workers': 1, 'pin_memory': True}
    # train_loder = DataLoader(train, batch_size=1, shuffle=False)
    # for i in train_loder:
    #     print(i.x.size())
    # print(train.transform,test.transform)
    # for i in dataset:
    #     print(i.id)
    # print(len(dataset))
