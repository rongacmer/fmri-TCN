...
class ResGCN(torch.nn.Module):
    ...
    def __init__(self, hidden, num_feat_layers=1, num_conv_layers=3,
                 num_fc_layers=2, gfn=False, collapse=False, residual=False,
                 res_branch="BNConvReLU", global_pool="sum_gating", dropout=0.5,
                 edge_norm=False,num_feature=130,num_class=2):
        ...
        if collapse:
            ...
            for i in range(num_fc_layers - 1):
                ...
                hidden_in = hidden
            self.lin_class = Linear(hidden_in, num_class)
            self.node_lin_class = Linear(hidden_in,num_class)
        else:
            self.bn_feat = BatchNorm1d(hidden_in)
            ...
            self.lin_class = Linear(hidden, num_class)
            self.node_lin_class = Linear(hidden, num_class)
        ...
        self.proj_head = nn.Sequential(nn.Linear(64, 64))
    ...
    def forward(self, data):
        ...
        if self.collapse:
            ...
            return self.forward_BNConvReLU(x, edge_index, batch, edge_attr, xg)
    ...
    ### This one!!!
    def forward_BNConvReLU(self, x, edge_index, batch, edge_attr, xg=None):
        x = self.bn_feat(x)
        x = F.relu(self.conv_feat(x, edge_index,edge_weight=edge_attr))
        for i, conv in enumerate(self.convs):
            x_ = self.bns_conv[i](x)
            x_ = F.relu(conv(x_, edge_index, edge_weight = edge_attr))
            x = x + x_ if self.conv_residual else x_
        x = F.dropout(x, p=self.dropout, training=self.training)
        node_x = self.node_lin_class(x)
        gate = 1 if self.gating is None else self.gating(x)
        x = self.global_pool(x * gate, batch)
        x = x if xg is None else x + xg
        for i, lin in enumerate(self.lins):
        ...
        if self.dropout > 0:
            x = F.dropout(x, p=self.dropout, training=self.training)
        x = self.lin_class(x)
        return F.log_softmax(x, dim=-1),F.log_softmax(node_x,dim=-1)
    ...
    def forward_cl(self, data):
        ...
        return self.forward_BNConvReLU_cl(x, edge_index, edge_attr, batch, xg)

    def forward_BNConvReLU_cl(self, x, edge_index, edge_attr, batch, xg=None):
        x = self.bn_feat(x)
        x = F.relu(self.conv_feat(x, edge_index,edge_weight=edge_attr))
        for i, conv in enumerate(self.convs):
            x_ = self.bns_conv[i](x)
    ...
    def forward_BNReLUConv(self, x, edge_index, edge_attr, batch, xg=None):
        x = self.bn_feat(x)
