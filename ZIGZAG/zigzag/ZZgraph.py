...
path = '/home/gpusharedata/rhb/fmri/data/PostProcessing/Graph/006_S_4153_2/'

# %% Parameters
nameFolderNet = path + 'P_index_'
...
Graphs = []
for i in range(starttime, starttime + sizeWindow):
...
for i in range(0, sizeWindow):
    ...
    for start in range(NVertices):
        for end in range(NVertices):
            g.add_edge(start, end, weight=1 - Graphs[i][start][end])
