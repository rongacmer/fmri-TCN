...
class fmriDataset(Dataset):
    ...
    def processed_file_names(self):
        return ['data_{}.pt'.format(i) for i in range(self.getlen())]
    ...
    def __load_data(self,subject_directory,y,subject_name):
        ...
        subject_data = []
        # subject_id = re.findall('(\d+)_S_(\d+)', subject_name) #subject_id ADNI
        # ts = re.findall('\d+_S_\d+_(\d+)',subject_name) #时间点的数据
        ######### ASD############
        subject_id = re.findall('.*_(\d\d+)',subject_name) #ASD,UM_1数据
        ts = ['0']
        #############################
        ...
        for i in range(self.time_lenght): #遍历所有时间点
            ...
            print(roi_stat.shape,p_value_network.shape)
    ...
    def get(self,idx):
        ...
        if self.state == 'processing':
            if subject_name not in self.cache:
                ...
                data['x'] = (data['x'] - torch.mean(data['x']))/(torch.std(data['x'])+esp)
                self.cache[subject_name] = [edge_index.astype(np.long),edge_attr.astype(np.float32),data['x']] #载入缓存
            # r = 0
            ...
            return {
                ...
                'x':self.cache[subject_name][2][r:r+self.time_slice],
                'y':data['y'],
                'id':data['id']
            }
    ...
    def process(self):
        ...
        for label,cat in enumerate(self.clf):
            ...
            for s in subject_txt:
                ...
                torch.save(subject_data,os.path.join(self.processed_dir,f'data_{idx}.pt'))
                print('{} save'.format(s))
                idx += 1
                # cnt += 1
                # if cnt >= 3:
